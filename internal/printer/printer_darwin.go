...
// darwinService implements the Service interface for macOS using CUPS/lp commands
type darwinService struct{}

// newPlatformService creates a new macOS printer service
func newPlatformService() Service {
	return &darwinService{}
}

// List returns all available printers on macOS using lpstat
func (s *darwinService) List() ([]PrinterInfo, error) {
...
// GetDefault returns the default printer name on macOS
func (s *darwinService) GetDefault() (string, error) {
...
func (s *darwinService) Status(printerName string) (*PrinterInfo, error) {
...
func (s *darwinService) parseStatus(line string) PrinterStatus {
...
func (s *darwinService) Print(job *PrintJob) error {
...
func (s *darwinService) PrintRaw(printerName string, data []byte) error {
	// Normalize printer name
	cupsName := strings.ReplaceAll(printerName, " ", "_")
...
func (s *darwinService) GetPrinterURI(printerName string) (string, error) {
	cupsName := strings.ReplaceAll(printerName, " ", "_")
...
func findPrinterDevice(printerName string) (string, error) {
	// Common USB printer device paths on macOS
	patterns := []string{
		"/dev/usb/lp*",
		"/dev/cu.usbserial*",
	}
